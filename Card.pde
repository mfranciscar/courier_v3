class Card {
 ...
 // Propietats
 PImage img;
 String title, director, date, genere;
 ...
 Card(String title, String director, String date, String genere){
   this.title = title;
 ...
 void display(float x, float y, float w, float h, float b){
   ...
   text(director, x + w/3 + w/6, y + 2*h/5);
   
   // Secció
   fill(0); textSize(18); textAlign(CENTER);
   text(date, x + 2*w/3 + w/6, y + 2*h/5);
   
   // Descripció
   fill(0);textSize(14); textAlign(LEFT);
   text(genere, x + w/3 + b, y + 2*h/3 - b, 2*w/3 - b*2, h/4);
